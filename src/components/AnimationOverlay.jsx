...
// ─── Durées d'affichage (ms) — SEUL endroit à modifier pour ajuster ──────────
// La valeur pilote à la fois le démontage React et les animations CSS
// (via --anim-duration) : ouverture, maintien et fermeture du bandeau se
// recalent proportionnellement (keyframes en %).
...
export default function AnimationOverlay({ theme = "neon" }) {
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(
      () => setAnim(null),
      ANIM_DURATION_MS[isWin ? "win" : "goal"],
    );
  ...
  return (
    <div
      ...
      style={{
        "--anim-duration": `${ANIM_DURATION_MS[isWin ? "win" : "goal"]}ms`,
      }}
    >
...
function GoalContent({ meta, isMyGoal, theme }) {
  return (
    <>
      {isMyGoal && theme === "neon" && (
        <div className="anim-sparks" aria-hidden="true" />
      )}
      <div
        className={`anim-label ${isMyGoal ? "anim-label--mine" : "anim-label--neutral"}`}
      >
        BUT
      </div>
